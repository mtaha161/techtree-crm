  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        {open ? '✕' : <img src="/logottwhite.png" style={{ width: 30, height: 30, objectFit: 'contain' }} />}
      </button>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ background: 'linear-gradient(135deg, #6366F1, #4338CA)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,.2)', display: 'grid', placeItems: 'center', flexShrink: 0 }}><img src="/logottwhite.png" style={{ width: 24, height: 24, objectFit: 'contain' }} /></div>
            <div style={{ flex: 1 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {messages.map((msg, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row', gap: 8, alignItems: 'flex-end' }}>
                {msg.role === 'ai' && (
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg,#6366F1,#4338CA)', display: 'grid', placeItems: 'center', flexShrink: 0 }}><img src="/logottwhite.png" style={{ width: 18, height: 18, objectFit: 'contain' }} /></div>
                )}
